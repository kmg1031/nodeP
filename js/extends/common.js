...
function showMsg2(msg,focus_id){
    $("#f_id").val(focus_id);
    var $alert = (parent) ? $('#alertModal', parent.document) : $('#alertModal');
    dc('## msg: '+msg);
//	focus_id=focus_id;
    ...
    // msg jsone format이 html 에도 적용되도록 형태 보존
    msg = '<pre class="alertPre">' + msg + '</pre>';
    ...
    $alert.on('hide.bs.modal',function(e){
        ...
        setTimeout(function(){setFocus($("#f_id").val());},1);
    });

}
function showMsg_list(msg){
    ...
    $alert.on('hide.bs.modal',function(e){
        e.stopImmediatePropagation();
        var foc_id=$("#f_id").val();
        setTimeout(function(){setFocus(foc_id);},1);
    });
 ...
 * 은행거래고유번호 세팅
 */
function fnMakeApiTranId(){

    var tran_dtime="";
    if(client_use_code==""||client_use_code==null||client_use_code=="null"){

    }else{
        //console.log('-------------------------------------------')
        //console.log(client_use_code)
        tran_dtime =client_use_code+"U0"+ new Date().format("ddHHmmss");
        $("input[name=bank_tran_id]").val(tran_dtime);
    }
 ...
 * 은행거래고유번호 다중 세팅
 */
function fnMakeApiTranId_multi(i){
    var tran_dtime="";

    if(client_use_code==""||client_use_code==null||client_use_code=="null"){
        return;
    }else{
        $(".bank_tran_id").each(function(){
            i++;
            if(i==21){
                i++;
            }
            var asciCode=String.fromCharCode(i+64);
            tran_dtime =client_use_code+"U"+ new Date().format("ddHHmmss")+asciCode;
            $(this).val(tran_dtime);
        });
    }
...
function sessionSetup(){
    ...
    $.ajax({
        ...
        .done(function(data, textStatus, jqXHR){
            ...
            scope			=data.scope;
            client_use_code	=data.client_use_code;
            user_seq_no		=data.user_seq_no;
            user_info		=data.user_info;

            if(data.client_use_code != ""){
                client_use_code_2 = data.client_use_code;
            }

            retVal = access_token;

            var str="";
            var json=data.access_token;
            str = "<option value=''>- 선택 - </option>";
            if(json!=null){
                for(var i=0;i<Object.keys(json).length;i++){
                    str += "<option value='"+json[i].access_token+"'>"+json[i].access_token+"("+json[i].scope+")</option>";
                }
            }else{
                str="<option value=''>사용자인증, 토큰발급 메뉴에서 [토큰발급] 또는 [등록계좌조회]를 실행하세요.</option>";
            }
...
function session_fin_Setup(scope_prm){
    $.ajax({
        url: '/getSession',
        type: 'post',
        dataType: 'json'
        ...
        .done(function(data, textStatus, jqXHR){
            ...
            if(json!=null){
                ...
                for(var i=0;i<Object.keys(json).length;i++){
                    console.log(scope_prm)
                    if(json[i].scope==scope_prm){//sa,oob 등등

                        var finList=json[i].finList;
//					finList=finList.split("\\[")[0];

                        if(finList!=null && finList !="" ){
                            var tmp = "";
                            for(var j=0;j<Object.keys(finList).length;j++){
                                tmp = finList[j].split("[")[0];
                                tmp = tmp.trim();
                                str += "<option value='"+ tmp +"'>"+finList[j]+"</option>";
                            }
                            // tmp = tmp.trim();
                            // str += "<option value=''>선택안함</option>";
                            $("#fintech_use_num").html(str);
                            return ;

                        }else{
                            str="<option value=''>등록된 핀테크 번호가 존재하지 않습니다.</option>";
                        }
                    }
                }
                if(json.finList==null){
                    str="<option value=''>등록계좌조회를 통해 조회해주세요.</option>";
                }
            }else{
                str="<option value=''>등록계좌조회를 통해 조회해주세요.</option>";
            }
...
function session_fin_Setup_dps(scope_prm){
    $.ajax({
        url: '/getSession',
        type: 'post',
        dataType: 'json'
    ...
    $.ajax({
        ...
        .done(function(data, textStatus, jqXHR){
            ...
            if(json!=undefined){
                ...
                for(var i=0;i<Object.keys(json).length;i++){
                    if(json[i].access_token==$("#token option:selected").val()){
                        client_use_code=json[i].client_use_code;
                        //console.log(client_use_code)
                        //				return json[i].client_use_code;
                    }

                    if(json[i].client_use_code!=null && json[i].client_use_code!="" && json[i].client_use_code!='null'){
                        tmp = json[i].client_use_code;
                    }
                }

                //client_use_code 값이 없을때 다른값으로 대체
                if(client_use_code==null || client_use_code=="" || client_use_code=='null'){
                    client_use_code = tmp;
                }
            }

//		return (data.client_use_code);
        })
        .fail(function(data,textStatus,jqXHR){
            console.log(data)
        });
 ...
 * 입력폼 행 추가 일부변경
 */
function addRow_cjs($jo){

    var cnt_all=$(".chk").length;
    console.log(cnt_all)
    if(cnt_all>24){
        showMsg("최대 25개까지 이용 가능합니다.");
        return false;
    }
...
function delRow_sel(){
    ...
    var cnt=1;
    var cnt_all=$(".chk").length;
    var cnt_sel=$(".chk:checked").length;

    if(cnt_all==cnt_sel){
        showMsg("적어도 한개의 요청리스트가 필요합니다.");
        return false;
    }
